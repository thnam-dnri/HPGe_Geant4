...
: G4VUserDetectorConstruction(),
  ...
  fLeadTopLV(nullptr), fCopperTopLV(nullptr), fLeadBottomLV(nullptr), fCopperBottomLV(nullptr)
{
}
...
G4VPhysicalVolume* DetectorConstruction::DefineVolumes()
{
    ...
    // ---------------------------------------------------------------------
    G4LogicalVolume* detectorMotherLV = fWorldLV;

    if (kEnableLeadShield) {
        const G4double leadInnerR = fShieldInnerRadius;
        const G4double leadOuterR = fShieldOuterRadius;
        const G4double leadHeight = fShieldHeight;
        const G4double leadCoverThick = fShieldThickness;
        ...
        const G4double cuOuterR = leadInnerR;
        const G4double cuInnerR = std::max(0.0, (cuOuterR - fCopperLinerThickness));
        const G4double cuHeight = std::max(0.0, (leadHeight - 2.0*mm));
        ...
        const G4double cavityR = cuInnerR;
        const G4double cavityH = leadHeight; // full height cavity
        ...
        new G4PVPlacement(0,
                          ...
                          fWorldLV,
                          false, 0, true);
        detectorMotherLV = fShieldCavityLV;
        ...
        new G4PVPlacement(0,
                          G4ThreeVector(0, 0, fShieldCenterZ + (leadHeight/2 + leadCoverThick/2)),
                          fLeadTopLV,
                          "LeadTopCover",
                          fWorldLV,
                          false, 0, true);
        ...
        const G4double cuTopThick = fCopperLinerThickness;
        ...
        const G4double cuBottomThick = fCopperLinerThickness;
        ...
        fLeadShieldLV = nullptr;
        fCopperLinerLV = nullptr;
        fShieldCavityLV = nullptr;
        fLeadTopLV = nullptr;
        fCopperTopLV = nullptr;
        fLeadBottomLV = nullptr;
        fCopperBottomLV = nullptr;
    }
    ...
    new G4PVPlacement(0, G4ThreeVector(0, 0, housingZ), housingLV, "Housing", detectorMotherLV, false, 0, true);
    ...
    new G4PVPlacement(0, G4ThreeVector(0, 0, housingZ), vacuumLV, "VacuumInside", detectorMotherLV, false, 0, true);
    ...
    if (kEnableLeadShield) {
        ...
        if (fLeadShieldLV) { fLeadShieldLV->SetVisAttributes(leadVis); }
        if (fLeadTopLV) { fLeadTopLV->SetVisAttributes(leadVis); }
        if (fLeadBottomLV) { fLeadBottomLV->SetVisAttributes(leadVis); }
        ...
        if (fCopperLinerLV) { fCopperLinerLV->SetVisAttributes(copperVis); }
        if (fCopperTopLV) { fCopperTopLV->SetVisAttributes(copperVis); }
        if (fCopperBottomLV) { fCopperBottomLV->SetVisAttributes(copperVis); }
